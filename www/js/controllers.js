...
.controller('PerfilCtrl', function($scope, $ionicPopup, $ionicLoading, $cordovaCamera, $webSql, $http, $ionicModal, $rootScope, $location, $state, $localStorage) {
  ...
  $scope.tmpPerfil = { firstName: $scope.growify.firstName, email: $scope.growify.email, password: $scope.growify.password };

  $scope.getProfile = function() {
    $scope.showload();
    $http.get($localStorage.growify.rest+'/profile', {
      headers: { 'x-access-token': $localStorage.growify.access_token }
    }).
    then(function (data, status, headers, config) {
      if (data.data.avatar) {
        $scope.fotoSubida = data.data.avatar;
      }
      else {
        $scope.fotoSubida = "img/unknown.png";
      }
  ...
  $scope.saveProfile = function() {
    $scope.showload();
    var postData = {
      firstName: $scope.tmpPerfil.firstName,
      email: $scope.tmpPerfil.email,
      password: $scope.growify.password
    };
    if ($scope.fotoSubida != "img/unknown.png") {
      postData.avatar = $scope.fotoSubida;
    }
    $http.put($localStorage.growify.rest+'/profile', postData, { headers: { 'x-access-token': $localStorage.growify.access_token } }).
    then(function (data, status, headers, config) {
      $scope.hideload();
      ok('Perfil actualizado con éxito');
      $localStorage.growify.firstName = $scope.tmpPerfil.firstName;
      $localStorage.growify.email = $scope.tmpPerfil.email;
      $localStorage.growify.password = $scope.tmpPerfil.password;
  ...
  $scope.reloadFav = function() {
    ...
    $rootScope.db.executeQuery("SELECT * FROM FAV WHERE userEmail = ?", [$localStorage.growify.email]
      ...
      if ($scope.totalFavoritos == 0) {
        $scope.hideload();
        $scope.isLoaded = true;
        $scope.totalFavoritosTiendas = 0;
        $scope.totalFavoritosProductos = 0;
        $scope.totalFavoritos = 0;
      }
      else {
        p = [];
